    ...
    process {
        ...
        if ($_ -match "\s*(?<location>(?<file>[^>]*)\((?<line>\d+),(?<column>\d+)\)): (?<title>(?<type>error|warning|message|note)[^:]*): (?<message>.*)") {
            if ($Matches.type -in @('error', 'warning')) {
                ...
                # This probably means that we got confused by interspersed logs,
                # and since there are long strings of notes after an error, we
                # would quickly exhaust our limit of 10 annotations.
                # Write down that we noticed it for debugging it, but don’t
                # make it an actual annotation so as not to spam.
